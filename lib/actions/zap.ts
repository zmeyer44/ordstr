...
const UserLud16Schema = z.object({
  lud16: z.string(),
});
...
export async function checkPayment(
  ...
  console.log("Price", parseInt(getTagValues("price", event.tags)));
  return (
    nostrPubkey === paymentEvent.pubkey &&
    invoice.amount >= parseInt(getTagValues("price", event.tags))
  );
}
export async function updateListUsersFromZaps(
  ...
  let validUsers: string[][] = currentUsers.filter(
    ([pubkey, relay, petname, expiryUnix]) =>
      parseInt(expiryUnix) > unixTimeNowInSeconds(),
  );
